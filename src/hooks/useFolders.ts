...
export const useMoveVideos = (videoId: string, currentWorkspace: string) => {
  ...
  //   states to fetch things
  const [isFetching, setIsFetching] = useState(false);

  // have a state for folders
  ...
  // use mutation data -> this will help us create optimistic change
  ...
  // zod forms-> it uses the hook useZodForms
  const { errors, onFormSubmit, watch, register } = useZodForm(
    moveVideoSchema,
    mutate,
    { folder_id: null, workspace_id: null }
  );
...
//   this useEffect will watch for the changes and everytime a change occurs, we want to fetch the folders again
  useEffect(()=> {
