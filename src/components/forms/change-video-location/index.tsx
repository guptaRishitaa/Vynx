...
{folder ? <p>{folder.name}</p> : "This video has no folder"}
        ...
        <div className='p-5 flex flex-col gap-y-5 border-[1px] rounded-xl'>
...
<option className='text-[#a4a4a4]' key={folder.id} value={folder.id}>
    {folder.name}
</option>
) : (
<option className='text-[#a4a4a4]' key={folder.id} value={folder.id}>
...
<p className='text-[#a4a4a4] text-sm'> This workspace has no folders</p>
                )}
                </Label>}
        </div>
        <Button>
